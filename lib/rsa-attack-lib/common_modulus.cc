...
namespace comodulus{
  ...
  mpz_class common_modulus_attack(RSA_data m_1, RSA_data m_2){
    ...
    if(m_1.n != m_2.n || ee_result[2] != 1){
      cerr << "Common modulus won't work. Both n_1 and n_2 must be the same and gcd(e_1, e_2) must equal 1. " << endl;
      return -1;
    }
    ...
    mpz_powm_ui(M.get_mpz_t(), product_M_e12.get_mpz_t(), 1, m_1.n.get_mpz_t()); //mods by n and assigns to M
